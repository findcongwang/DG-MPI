...
{
  theFieldsCoefficients = alloc_2d_double(Nb, NbUnknowns * FunctionSpaceSize);
}
...
void DG_Dofs::print(int id = -1)
{
    printf("Cell: %d NU: %d, Nb: %d, FS: %d\n", id, NU, Nb, FS);
    for(int i = 0; i < Nb; ++i)
    {
        for(int j = 0; j < FS*NU; ++j)
        {
            cout << theFieldsCoefficients[i][j] << " ";
        }
        cout << endl;
    }
}
