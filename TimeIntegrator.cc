...
void TimeIntegrator::syncParititionBoundaries(double t)
{
	MPI_Barrier(MPI_COMM_WORLD);

	//sends all bordered to sync
	//printf("[SYNC [%d]: Started receiving]\n", rank);
	...
	for(it = mesh_begin;it != mesh_end;++it)
	{
		...
		if(m->neighbours.size() > 0 && m->partition != myPartitionID)
		{
			int rc1, rc2, rc3;

/*			MPI_Status s;
			int testnum;
			MPI_Recv(&testnum, 1, MPI_INT, m->partition-1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			printf("RECEIVED TESTNUM %d\n", testnum);*/

			//printf("[SYNC [%d]: Receiving element %d]\n", rank, m->elmID);

			rc1 = MPI_Recv(&cell->theRightHandSide[0], cell->fSize * cell->cSize, MPI_DOUBLE, 
						m->partition-1, m->elmID * 10 + 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			rc2 = MPI_Recv(&cell->theMean[0], cell->cSize, MPI_DOUBLE,
						m->partition-1, m->elmID * 10 + 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			rc3 = MPI_Recv(&cell->theFieldsCoefficients->theFieldsCoefficients[0][0],
						cell->theFieldsCoefficients->Nb *
						cell->theFieldsCoefficients->NU *
						cell->theFieldsCoefficients->FS, MPI_DOUBLE, 
						m->partition-1, m->elmID * 10 + 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

			if(rc1 != MPI_SUCCESS || rc2 != MPI_SUCCESS || rc3 != MPI_SUCCESS)
				printf("something failed\n");

			//cell->theFieldsCoefficients->print(m->elmID);
		}
	}

	reqs.clear();
	//printf("[SYNC [%d]: complete for time %f]\n", rank, t);
}
...
/*				MPI_Request r;
				int testnum = m->elmID;
				MPI_Isend(&testnum, 1, MPI_INT,
							m->neighbours[i]-1, 1, MPI_COMM_WORLD, &r);*/

				reqs.resize(reqs.size() + 3);
				stats.resize(stats.size() + 3);
				//printf("[SYNC [%d]: Sending element %d to %d]\n", rank, m->elmID, m->neighbours[i]-1);

				MPI_Isend(&cell->theRightHandSide[0], cell->fSize * cell->cSize, MPI_DOUBLE,
							m->neighbours[i]-1, m->elmID * 10 + 0, MPI_COMM_WORLD, &reqs[reqs.size()-3]);
				MPI_Isend(&cell->theMean[0], cell->cSize, MPI_DOUBLE,
							m->neighbours[i]-1, m->elmID * 10 + 1, MPI_COMM_WORLD, &reqs[reqs.size()-2]);
				MPI_Isend(&cell->theFieldsCoefficients->theFieldsCoefficients[0][0], 
							cell->theFieldsCoefficients->Nb *
							cell->theFieldsCoefficients->NU *
							cell->theFieldsCoefficients->FS, MPI_DOUBLE, 
							m->neighbours[i]-1, m->elmID * 10 + 2, MPI_COMM_WORLD, &reqs[reqs.size()-1]);
				
				//cell->theFieldsCoefficients->print(m->elmID);
			}
		}	
	}
	syncParititionBoundaries(t);
}
