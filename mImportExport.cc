...
void mImportExport::importGmshFile (char *fName, mMesh *theMesh)
{
	...
	while(!feof(f))
	{
	  fscanf(f,"%s",line);
		...
		{
			importGmshFilev2(fName, theMesh);
			return;
		}
		printf("support for version 1 is dropped due to partitions, exiting\n");
		exit(0);
...
void mImportExport::importGmshFilev2 (char *fName, mMesh *theMesh)
{
	ifstream meshFile(fName);
	string junk;
	int junkint;
    int numNode, numElm;
    meshFile >> junk >> junk >> junk >> junk >> junk >> junk >> numNode;

    theMesh->resize(0,numNode);
    
    for(int i = 0; i < numNode; ++i)
    {
    	int nodeID;
    	double x, y, z;
    	meshFile >> nodeID >> x >> y >> z;
    	theMesh->createVertex(nodeID,x,y,z,0);
    }	

    meshFile >> junk >> junk >> numElm;

    for(int i = 0; i < numElm; ++i)
    {
    	int elmID, elmType, numTags, elmGroup, elmZone;
    	int numParts, elmPart, elmNumNode;
    	vector<int> neighbours;
    	bool keepElm = false;

    	meshFile >> elmID >> elmType >> numTags >> elmGroup >> elmZone;
    	meshFile >> numParts >> elmPart;
    	for(int i = 0; i < numParts-1; ++i)
    	{
    		int neigh;
    		meshFile >> neigh;
    		neighbours.push_back(abs(neigh));

    		if(abs(neigh) == myPartitionID) keepElm = true;
    	}

    	if(elmPart == myPartitionID) keepElm = true;

    	/*if(!keepElm)
    	{
    		//not this partition, skip
    		getline(meshFile, junk);
    		continue;
    	}*/

    	int geomOrder = decodeGeomOrder(elmType);
		vector<int> nodeVec;
		...
		case 1 : 
			elmNumNode = 2;
			nodeVec.resize(elmNumNode, 0);
			for (int i = 0; i < elmNumNode; i++) meshFile >> nodeVec[i];
			theMesh->add(theMesh->createEdgeWP(nodeVec,geomOrder,theMesh->getGEntity(elmGroup,1), elmPart, neighbours, elmID));
		  	break;
		case 9  :	// triangle
			elmNumNode = 6;
		case 2  :
			...
			nodeVec.resize(elmNumNode, 0);
			for (int i = 0; i < elmNumNode; i++) meshFile >> nodeVec[i];
			theMesh->add(theMesh->createFaceWithVerticesWP(nodeVec,2,geomOrder,theMesh->getGEntity(elmGroup,2), elmPart, neighbours, elmID));
		  	break;
		case 10: 
			elmNumNode = 9;
		case 3 : // quad
			...
			nodeVec.resize(elmNumNode, 0);
			for (int i = 0; i < elmNumNode; i++) meshFile >> nodeVec[i];
			theMesh->add(theMesh->createFaceWithVerticesWP(nodeVec,3,geomOrder,theMesh->getGEntity(elmGroup,2), elmPart, neighbours, elmID));
		  	break;
		case 11: 
			elmNumNode = 10;
		case 4 : // tetraedron
			...
			nodeVec.resize(elmNumNode, 0);
			for (int i = 0; i < elmNumNode; i++) meshFile >> nodeVec[i];
			theMesh->add(theMesh->createTetWithVerticesWP(nodeVec,geomOrder,theMesh->getGEntity(elmGroup,3), elmPart, neighbours, elmID));
		  	break;
		case 5 :// Hex
		    int iNod1,iNod2,iNod3,iNod4,iNod5,iNod6,iNod7,iNod8;
		    meshFile >> iNod1 >> iNod2 >> iNod3 >> iNod4 >> iNod5 >> iNod6 >> iNod7	>> iNod8;
		    theMesh->createHexWithVertices(iNod1, iNod2,iNod3,iNod4,
						   iNod5, iNod6,iNod7,iNod8,
						   theMesh->getGEntity(elmGroup,3));
		  	break;
		default:
			printf("This type of element is not recognized ");
			cout << elmType << endl;
