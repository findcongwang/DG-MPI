...
Edge *mMesh::createEdgeWP (vector<int>& vertexiD, int geomOrder, gEntity *classif,
     int elmPartition, vector<int>& elmNeighbours, int elmID)
{
  Edge *theNewEdge;
  vector<Vertex*> v;
  vector<int>::iterator it;
...
#endif
    theNewEdge = new Edge(v,geomOrder,classif);
    theNewEdge->partition = elmPartition;
    theNewEdge->neighbours = elmNeighbours;
    theNewEdge->elmID = elmID;
  return theNewEdge;
}
...
Face *mMesh::createFaceWithVertices(vector<int>& vertexiD, int theType, int geomOrder, 
  gEntity *classif)
{
  Face *theNewFace;
  vector<Vertex*> v;
...
Face *mMesh::createFaceWithVerticesWP(vector<int>& vertexiD, int theType, int geomOrder, 
  gEntity *classif, int elmPartition, vector<int>& elmNeighbours, int elmID)
{
  Face *theNewFace;
  vector<Vertex*> v;
  vector<int>::iterator it;
...
#endif
  theNewFace = new Face(v,theType,geomOrder,classif);
  theNewFace->partition = elmPartition;
  theNewFace->neighbours = elmNeighbours;
  theNewFace->elmID = elmID;
  return theNewFace;
}
...
mTet *mMesh::createTetWithVerticesWP (vector<int>& vertexiD, int geomOrder, gEntity *classif,
  int elmPartition, vector<int>& elmNeighbours, int elmID)
{
  mTet *theNewTet;
  vector<Vertex*> v;
  ...
  theNewTet = new mTet(v,geomOrder,classif);
  theNewTet->partition = elmPartition;
  theNewTet->neighbours = elmNeighbours;
  theNewTet->elmID = elmID;
  return theNewTet;
}
